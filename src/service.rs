...
use web3::types::{BlockId, BlockNumber, SyncState, TxpoolStatus};

lazy_static! {
    static ref GETH_VERSION: IntGaugeVec =
        register_int_gauge_vec!("geth_version", "Client version", &["value"]).unwrap();
    static ref GETH_NETWORK: IntGaugeVec =
        register_int_gauge_vec!("geth_network", "Client network", &["value"]).unwrap();
    static ref GETH_LATEST: IntGaugeVec =
        register_int_gauge_vec!("geth_latest", "Latest block information", &["hash"]).unwrap();
    ...
    .unwrap();
    static ref GETH_GAS_PRICE: IntGauge =
        register_int_gauge!("geth_gas_price", "Current gas price in wei").unwrap();
    static ref GETH_MEMPOOL_SIZE: IntGaugeVec =
        register_int_gauge_vec!("geth_mempool_size", "Mempool information", &["type"]).unwrap();
    static ref GETH_PEERS: IntGaugeVec =
        register_int_gauge_vec!("geth_peers", "Connected peers", &["version"]).unwrap();
...
pub async fn update_metrics(node: &String) -> Result<(), web3::Error> {
    ...
    GETH_MEMPOOL_SIZE
        ...
        .set(mempool.queued.as_u64().try_into().unwrap());

    let peers = web3.net().peer_count().await?;
    GETH_PEERS
        .with_label_values(&["all"])
        .set(peers.as_u64().try_into().unwrap());
